...
const Rating = () => {
    ...
    return (
        <div className='w-full h-auto pb-20 pt-6 justify-center items-center flex'>
            <div className='sm:w-[90%] w-full md:px-0 px-3 h-auto flex-wrap gap-12 md:gap-5 justify-evenly items-center flex'>
                {rating_data.map(({ rate, type, title }, index) => (
                    <div key={index} className='flex justify-center items-center w-auto h-auto gap-4 relative p-1 xs:p-3'>
                        ...
                        <div className='w-full h-full gap-3 bg-transparent flex flex-col justify-center items-center'>
                            <h2 className='sm:text-5xl text-4xl font-medium uppercase sm:px-2 px-1' >{rate}{type}</h2>
